        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t, idx) => (
                <ScrollReveal key={idx} delay={idx * 0.1} direction="up" className="h-full">
                    <div className={`p-8 rounded-2xl border bg-gradient-to-b from-[#1a1a1a] to-[#111111] transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_30px_-10px_rgba(255,203,116,0.1)] group h-full flex flex-col ${idx === 1 ? 'border-[#FFCB74]/30 shadow-[0_0_30px_-10px_rgba(255,203,116,0.1)]' : 'border-[#2F2F2F]/50 opacity-90 hover:opacity-100 hover:border-[#FFCB74]/30'}`}>
                        <div className="flex gap-1 mb-6">
                            {[1,2,3,4,5].map(s => <Star key={s} size={14} className="text-[#FFCB74] fill-[#FFCB74]" />)}
                        </div>
                        <p className="text-sm leading-relaxed text-gray-300 mb-8 group-hover:text-[#F6F6F6] transition-colors flex-1">"{t.text}"</p>
